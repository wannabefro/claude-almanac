...
def test_upsert_sym_is_idempotent_by_symbol_key(tmp_path):
    p = _init(tmp_path)
    db.upsert_sym(p, kind="sym", text="v1", file_path="src/a.py",
                  symbol_name="foo", module="src",
                  line_start=1, line_end=1, commit_sha="sha1",
                  embedding=[1.0, 0.0])
    db.upsert_sym(p, kind="sym", text="v2", file_path="src/a.py",
                  symbol_name="foo", module="src",
...
def test_arch_upsert_is_keyed_by_module(tmp_path):
    p = _init(tmp_path)
    db.upsert_sym(p, kind="arch", text="old summary", file_path=None,
                  symbol_name=None, module="src",
                  line_start=None, line_end=None, commit_sha="sha1",
                  embedding=[1.0, 0.0])
    db.upsert_sym(p, kind="arch", text="new summary", file_path=None,
                  symbol_name=None, module="src",
...
def test_delete_by_file_removes_entries_and_vec(tmp_path):
    p = _init(tmp_path)
    db.upsert_sym(p, kind="sym", text="a", file_path="src/a.py",
...
def test_last_sha_returns_latest(tmp_path):
    p = _init(tmp_path)
    db.upsert_sym(p, kind="sym", text="x", file_path="src/a.py",
                  symbol_name="f", module="src",
                  line_start=1, line_end=1, commit_sha="sha1",
                  embedding=[1.0, 0.0])
